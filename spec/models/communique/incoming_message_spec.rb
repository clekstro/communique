...
describe Communique::IncomingMessage do
  # includes Readable
  ...
  it "is unread by default" do
    received_message = Communique::IncomingMessage.new
  ...
  describe "Class Methods" do
    subject { Communique::IncomingMessage }
    ...
    context "#read" do
      it "should show only read messages" do
        ...
        Communique::IncomingMessage.read.should include(read)
        Communique::IncomingMessage.read.should_not include(unread)
      end
    end
    ...
    context "#unread" do
      it "returns only unread messages" do
        read = create(:received_message, read: true, recipient_id: 1)
        unread = create(:received_message, read: false, recipient_id: 2)
        Communique::IncomingMessage.unread.should include(unread)
        Communique::IncomingMessage.unread.should_not include(read)
      end
    end
     ...
     context "#for" do
      it "should return only those messages received by recipient" do
        ...
        Communique::IncomingMessage.for(recipient).should include(received_message)
        Communique::IncomingMessage.for(other).should_not include(received_message)
      end
    end
  end
end
